...
const quickActions = [
  ...
  { label: "Delete Table", action: "delete_table", params: { catalogName: "", schemaName: "", tableName: "" } },
  { label: "Create Schema", action: "create_schema", params: { catalogName: "", name: "new_schema", comment: "" } },
  { label: "Grant Privileges", action: "grant_privileges", params: { privileges: "SELECT", principal: "user1", securable_type: "TABLE", full_name: "" } },
...
const TableModal = ({ type, table, schemaName, catalogName, onClose, onSubmit }) => {
  const [tableName, setTableName] = useState(table?.name || "");
  const [tableComment, setTableComment] = useState(table?.comment || "");
  const [columns, setColumns] = useState(
    table?.columns || [{ name: "", type: "STRING", nullable: true }]
  );
  const [metadataKeywords, setMetadataKeywords] = useState(
    table?.metadata?.keywords?.join(", ") || ""
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (columns.some((col) => !col.name || !col.type)) {
  ...
  const addColumn = () => {
    setColumns([...columns, { name: "", type: "STRING", nullable: true }]);
  };
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[800px] bg-white shadow-lg rounded-lg border border-indigo-100">
        <DialogHeader>
          <DialogTitle className="bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">
            {type === "delete" ? "Delete Table" : "Create New Table"}
          </DialogTitle>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <>
            <p className="mb-6 text-gray-700">
              ...
              <span className="font-semibold">{table?.name}</span>? This action
              cannot be undone.
            </p>
            {error && (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  A brief description of the table's purpose or content
                </p>
              </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                          <div>
                            ...
                            <Input
                              ...
                              className="border-indigo-100 focus:border-indigo-300 focus:ring-indigo-200 text-sm"
                              required
                            />
                          </div>
                          <div>
                            ...
                            >
                              <SelectTrigger className="border-indigo-100 focus:border-indigo-300 focus:ring-indigo-200 text-sm">
              ...
              <div>
                ...
                <Input
                  placeholder="Comma-separated tags (e.g., customer, sales, region)"
                  value={metadataKeywords}
                  onChange={(e) => setMetadataKeywords(e.target.value)}
                  className="border-indigo-100 focus:border-indigo-300 focus:ring-indigo-200"
                />
                <p className="text-xs text-gray-500 mt-1">
                  These tags will help your ML model discover and use this table
                </p>
              </div>
            </div>
...
export default function TablesPage() {
  ...
  const schemaName = queryParams.get("schema");
  ...
  const [chatMessages, setChatMessages] = useState([
    { sender: "bot", text: "Hello! Select an action from the dropdown or use voice commands to manage tables, schemas, privileges, or optimize your workspace." }
  ]);
  ...
  const handleModalSubmit = async (data) => {
    try {
      if (modalType === "delete") {
        await deleteTable(data.catalogName, data.schemaName, data.tableName);
        const updatedTables = await getTables(catalogName, schemaName);
  ...
  const filteredTables = tables.filter(
    (table) =>
      ...
      (table.metadata?.keywords?.some((keyword) =>
  ...
  const processVoiceCommand = (transcript) => {
    ...
    if (action.value === "create_table") {
      ...
      else params.schemaName = schemaName;
      ...
      else params.full_name = `${catalogName}.${schemaName}.${params.tableName || ''}`;
    }
  ...
  const handleChatSubmit = async (e) => {
    ...
    try {
      switch (selectedAction) {
        case "create_table": {
          const { catalogName: catalog, schemaName: schema, tableName, columns, comment, metadata } = actionParams;
          ...
          await createTable({ catalogName: catalog || catalogName, schemaName: schema || schemaName, tableData });
          setChatMessages((prev) => [
            ...prev,
            { sender: "bot", text: `Table '${tableName}' created in ${catalog || catalogName}.${schema || schemaName} successfully!` }
          ]);
          toast.success(`Table '${tableName}' created successfully!`);
          const updatedTables = await getTables(catalogName, schemaName);
          setTables(updatedTables);
        ...
        case "delete_table": {
          const { catalogName: catalog, schemaName: schema, tableName } = actionParams;
          await deleteTable(catalog || catalogName, schema || schemaName, tableName);
          setChatMessages((prev) => [
            ...prev,
            { sender: "bot", text: `Table '${tableName}' in ${catalog || catalogName}.${schema || schemaName} deleted successfully!` }
          ]);
          ...
          <div className="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm p-6 rounded-xl border border-indigo-50 shadow-sm">
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">
                Catalog or Schema Not Specified
              </h3>
              <p className="text-gray-600 mb-6">
                Please select a catalog and schema first
              </p>
              <Button
        ...
        <div className="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm p-6 rounded-xl border border-indigo-50 shadow-sm">
          ...
          >
            ...
            <div className="relative mb-6 max-w-md">
              ...
              <Input
                placeholder="Search tables or keywords..."
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[30%] text-center text-lg font-semibold text-indigo-600">
                        Table Name
                      </TableHead>
                      <TableHead className="w-[30%] text-center text-lg font-semibold text-indigo-600">
                        Description
                      ...
                      <TableHead className="w-[10%] text-center text-lg font-semibold text-indigo-600">
                        Actions
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {isLoading ? (
                      ...
                      filteredTables.map((table) => (
                        ...
                        >
                          <TableCell className="font-medium w-[30%] text-center text-gray-800">
                            {table.name}
                          </TableCell>
                          <TableCell className="w-[30%] text-center text-gray-600">
                            {table.comment || "-"}
                          </TableCell>
                          <TableCell className="w-[30%] text-center text-gray-600">
                            <div className="flex flex-wrap gap-2 justify-center">
                              {table.metadata?.keywords && table.metadata.keywords.length > 0 ? (
                                table.metadata.keywords.map((tag, index) => (
                                  <span 
                                    key={index}
            ...
            <Card className="p-6 bg-gradient-to-br from-indigo-50/50 to-indigo-100/20 border border-indigo-50">
              <CardContent className="text-center">
                ...
                <p className="text-gray-600 mt-2 mb-6">
                  Create a new table with columns and metadata keywords for ML
                  discovery
                </p>
                <motion.div
                ...
                >
                  ...
                  >
                    <Plus className="mr-2 h-5 w-5" /> Add New Table
                  </Button>
                </motion.div>
              </CardContent>
            </Card>
              ...
              >
                ...
                <div className="p-4 border-t border-indigo-200 bg-white">
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    >
                      <FaTrashIcon className="h-3 w-3 mr-1" />
                      Clear Chat
                    </Button>
                  </div>
