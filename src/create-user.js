...
import { Info, AlertCircle, ArrowLeft, Check, Search, Bell, Settings, LogOut, Send } from "lucide-react";
...
const entitlements = [
  {
    id: "CAN_MANAGE",
    title: "Admin Access",
    description: "Full administrative privileges including user management",
    permissions: [
  ...
  {
    id: "CAN_USE_WORKSPACE",
    title: "Workspace Access",
    description: "Standard workspace user permissions",
    permissions: [
  ...
  {
    id: "CAN_USE_SQL_ANALYTICS",
    title: "SQL Analytics Access",
    description: "Databricks SQL and dashboard permissions",
    permissions: [
...
export default function CreateUser() {
  ...
  useEffect(() => {
    const validateEmail = async () => {
      if (isValidEmail(form.email)) {
        setStatus(prev => ({ ...prev, emailChecking: true }));
        try {
          const user = await checkUserExists(form.email);
          setStatus(prev => ({
            ...prev,
            existingUser: user,
            emailChecking: false
    ...
    const timer = setTimeout(() => {
      if (form.email && isValidEmail(form.email)) validateEmail();
    }, 500);
  ...
  const handleChange = (e) => {
    ...
    if (name === "email") {
      ...
      setStatus(prev => ({ ...prev, existingUser: null, error: null }));
    }
  };

  const handleEntitlementChange = (entitlementId) => {
    setForm(prev => ({
      ...prev,
      entitlements: prev.entitlements.includes(entitlementId)
        ? prev.entitlements.filter(id => id !== entitlementId)
        : [...prev.entitlements, entitlementId]
    }));
  };
    ...
    try {
      switch (selectedAction) {
        case "create_user": {
          ...
          const userData = { email, displayName: displayName || email.split("@")[0], entitlements: form.entitlements };
          const newUser = await createUser(userData);
          setChatMessages((prev) => [
            ...prev,
            { sender: "bot", text: `User '${email}' created successfully!` }
          ]);
          setForm({ displayName: "", email: "", entitlements: [] });
  ...
  const renderedEntitlements = useMemo(() =>
    entitlements.map((entitlement) => (
      ...
      >
        <div className="flex items-start space-x-3 rounded-lg border border-indigo-50 p-4 hover:bg-indigo-50/30">
          <Checkbox
            id={entitlement.id}
            checked={form.entitlements.includes(entitlement.id)}
            onCheckedChange={() => handleEntitlementChange(entitlement.id)}
            disabled={status.loading}
            className="mt-1"
          />
          <div className="space-y-1">
            <Label htmlFor={entitlement.id} className="font-medium">
              {entitlement.title}
            </Label>
            ...
            <ul className="text-xs text-gray-500 mt-2 space-y-1">
              {entitlement.permissions.map((permission) => (
    ...
    )), [form.entitlements, status.loading]);
  ...
  return (
    <div className="min-h-screen p-6 relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-3xl mx-auto bg-white/90 backdrop-blur-sm rounded-xl border border-indigo-50 shadow-sm">
          <motion.div
          ...
          >
            <div className="p-6">
              ...
              <Card className="border border-indigo-50 shadow-sm">
                <CardHeader>
                  <CardTitle className="text-2xl font-bold text-center bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">
                    Create New User
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  {status.existingUser && (
                    <Alert className="mb-6 bg-blue-50 border-blue-200">
                      ...
                      <AlertTitle className="text-blue-800">User Exists</AlertTitle>
                      <AlertDescription className="text-blue-700">
                        <div className="space-y-1">
                          <p>This user already exists in the system.</p>
                          <div className="text-sm mt-2 space-y-1">
                            <p>
                              <span className="font-medium">Display Name:</span>{" "}
                              {status.existingUser.displayName}
                            </p>
                            <p>
                              <span className="font-medium">Groups:</span>{" "}
                              {status.existingUser.groups?.length > 0
                                ? status.existingUser.groups.map(g => g.name || g).join(", ")
                                : "None"}
                            </p>
                            <p>
                              <span className="font-medium">Entitlements:</span>{" "}
                              {status.existingUser.entitlements?.length > 0
                                ? status.existingUser.entitlements.join(", ")
                                : "None"}
                            </p>
                          </div>
                        </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                      ...
                      />
                      {touched.displayName && !form.displayName && (
                    ...
                    <div className="space-y-2">
                      ...
                      {emailValidated && !status.existingUser && form.email && (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {renderedEntitlements}
                      </div>
                      {touched.entitlements && form.entitlements.length === 0 && (
                        <p className="text-sm text-red-500">At least one entitlement is required</p>
                      )}
                    </div>
                    ...
                    <div className="pt-4">
                      ...
                      >
                        <Button
                          ...
                          size="lg"
                          disabled={status.loading || status.existingUser || !isFormValid()}
                        >
                          {status.loading ? (
                            <>
                              <svg
