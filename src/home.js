...
const DashboardPreview = ({ setIsDashboardOpen }) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, delay: 0.2 }}
      className="bg-white/90 backdrop-blur-sm p-4 rounded-xl border border-indigo-50 shadow-sm relative"
    >
      <div className="relative" style={{ filter: 'blur(4px)', opacity: 0.9 }}>
        <h3 className="text-lg font-semibold text-gray-800 mb-3">Workspace Dashboard</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          <Card className="bg-indigo-50">
            <CardContent className="p-3">
              <p className="text-sm font-semibold text-indigo-600">Active Queries</p>
              <p className="text-xl font-bold text-gray-800">120</p>
            </CardContent>
          </Card>
          <Card className="bg-indigo-50">
            <CardContent className="p-3">
              <p className="text-sm font-semibold text-indigo-600">Storage Usage</p>
              <p className="text-xl font-bold text-gray-800">1.2 TB</p>
            </CardContent>
          </Card>
          <div className="md:col-span-2 bg-gray-100 rounded-lg p-3">
            <div className="h-24 bg-gray-200 rounded relative">
              <div className="absolute bottom-0 left-8 w-6 h-16 bg-indigo-400 rounded-t"></div>
              <div className="absolute bottom-0 left-16 w-6 h-20 bg-indigo-500 rounded-t"></div>
              <div className="absolute bottom-0 left-24 w-6 h-14 bg-indigo-600 rounded-t"></div>
            </div>
          </div>
          ...
          <Card className="bg-indigo-50">
            <CardContent className="p-3">
              <p className="text-sm font-semibold text-indigo-600">Recent Activity</p>
      ...
      <div className="absolute inset-0 bg-white/30 pointer-events-none rounded-xl"></div>
      <motion.div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      >
        <Button
            onClick={() => setIsDashboardOpen(true)}
            className="moving-gradient text-white px-8 py-4 text-lg rounded-lg transition-all shadow-lg hover:shadow-xl"
        >
            View All Assets
        </Button>
      </motion.div>
...
export default function HomePage() {
  ...
  const processVoiceCommand = (transcript) => {
    ...
    if (action.value === "create_user") {
      ...
      params.columns = '[{"name":"id","type":"int"}]'; // Default columns for voice command
    ...
    try {
      ...
      if (highStorageCatalogs.length > 0) {
        insights.push({
          ...
          description: `Catalog(s) ${highStorageCatalogs.map(c => c.name).join(', ')} are using ${highStorageCatalogs.reduce((sum, c) => sum + c.value, 0)} MB, which is more than 30% of total storage (${totalStorage} MB). This could lead to increased storage costs.`,
          recommendation: 'Review these catalogs for unused or old data. Consider archiving to a cheaper storage tier or optimizing data compression to reduce expenses.'
        });
      }
      ...
      if (querySpikes.length > 0) {
        insights.push({
          ...
          description: `On ${querySpikes.map(day => day.day).join(', ')}, query counts exceeded 100 (${querySpikes.map(day => day.queries).join(', ')}). High query volumes may impact performance and increase compute costs.`,
          recommendation: 'Consider load balancing by scheduling queries during off-peak hours or scaling up compute resources to handle peak loads efficiently.'
        });
      }
      ...
      if (engagementRate < 30) {
        insights.push({
          ...
          description: `Only ${activeUsers} out of ${totalUsers} users are active (${engagementRate.toFixed(1)}% engagement rate). Low engagement may indicate underutilization of resources.`,
          recommendation: 'Engage inactive users through training sessions or notifications about new features. If licenses are per-user, consider reducing the number of licensed users to cut costs.'
        });
      }
      ...
      if (excessivePrivileges.length > 0) {
        insights.push({
          ...
          description: `Privileges like ${excessivePrivileges.map(p => p.name).join(', ')} are assigned to more than 50% of users (${excessivePrivileges.map(p => p.value).join(', ')} users). This may pose a security risk.`,
          recommendation: 'Audit privilege assignments and follow the principle of least privilege. Remove unnecessary privileges to enhance security compliance and reduce risk.'
        });
      }
      ...
      if (problematicQueries.length > 0) {
        insights.push({
          ...
          description: `Found ${problematicQueries.length} problematic queries: ${problematicQueries.map(q => `'${q.query}' (${q.status === 'FAILED' ? 'Failed' : `Long-running: ${q.duration}`}) by ${q.user}`).join(', ')}. These may impact performance and user experience.`,
          recommendation: 'Optimize these queries by rewriting them for efficiency or indexing relevant tables. For failed queries, investigate the root cause and address any errors.'
        });
      }
  
      return insights.length > 0 ? insights : [{ severity: 'Low', title: 'No Critical Issues', description: 'Your workspace appears to be running smoothly.', recommendation: 'Continue monitoring usage and privileges to maintain efficiency.' }];
    } catch (error) {
      console.error('Error generating workspace insights:', error);
      return [{ severity: 'Error', title: 'Failed to Analyze Workspace', description: 'Unable to fetch workspace data.', recommendation: 'Please try again later or check the server logs for issues.' }];
    }
  };
    ...
    try {
      switch (selectedAction) {
        case "create_user": {
          ...
          await createUser({ email, displayName: displayName || email.split("@")[0] });
          setChatMessages((prev) => [
            ...prev,
        ...
        case "optimize_workspace": {
          setChatMessages((prev) => [
            ...prev,
            { sender: "bot", text: "Analyzing your workspace for optimization opportunities and business insights..." }
          ]);
    ...
    <div className="min-h-screen p-6 flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10">
        <nav className="flex items-center justify-between border-b border-indigo-100 pb-4 mb-6 px-6 bg-white/90 backdrop-blur-sm rounded-lg shadow-sm">
          ...
          <div className="flex-1 flex justify-center">
            <div className="flex items-center space-x-8">
              <NavLink to="/home" className="text-lg font-medium text-indigo-600 hover:text-indigo-800 transition-colors">Home</NavLink>
              <NavLink to="/users" className="text-lg font-medium text-gray-500 hover:text-indigo-600 transition-colors">Users</NavLink>
              <NavLink to="/groups" className="text-lg font-medium text-gray-500 hover:text-indigo-600 transition-colors">Groups</NavLink>
              <NavLink to="/catalog" className="text-lg font-medium text-gray-500 hover:text-indigo-600 transition-colors">Catalog</NavLink>
              <NavLink to="/grant" className="text-lg font-medium text-gray-500 hover:text-indigo-600 transition-colors">Grant</NavLink>
        ...
        <Dialog open={isDashboardOpen} onOpenChange={setIsDashboardOpen}>
          <DialogContent className="sm:max-w-[90vw] md:max-w-[1200px] max-h-[90vh] overflow-y-auto bg-white rounded-lg border border-indigo-100">
            <div className="p-6">
              <Dashboard />
            </div>
            <Button onClick={() => setIsDashboardOpen(false)} className="bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white">
              Close
            </Button>
          </DialogContent>
        </Dialog>
        ...
        <div className="flex-1 flex flex-col">
          <motion.div className="py-16 px-4 sm:px-6 lg:px-8 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <h1 className="text-5xl font-bold mb-6">
              <span className="bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">
                Introduction to Unity Catalog Command Center
              </span>
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Efficiently manage users, groups, and permissions with intuitive controls or voice commands.
            </p>
          </motion.div>

          <div className="flex-1 flex flex-col justify-center px-4 sm:px-6 lg:px-8">
            <motion.div className="mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }}>
              <DashboardPreview setIsDashboardOpen={setIsDashboardOpen} />
            </motion.div>

            <motion.h2 className="text-3xl font-semibold text-center mb-12 text-gray-800" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.2 }}>
              Feature List
            </motion.h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {features.map((feature, index) => (
                <motion.div
                  ...
                  whileHover={{ scale: 1.03, boxShadow: "0 10px 25px -5px rgba(99, 102, 241, 0.2)", transition: { duration: 0.3 } }}
                ...
                >
                  ...
                  <div className="relative z-10 bg-indigo-100 p-4 rounded-full mb-4 transition-all duration-300">
                    <feature.icon className="text-indigo-600 text-4xl transition-all duration-300" />
                  </div>
          ...
          <motion.div className="py-16 px-4 sm:px-6 lg:px-8 bg-white/90 backdrop-blur-sm mt-16 rounded-xl mx-4 sm:mx-6 lg:mx-8 border border-indigo-50 shadow-sm" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
            <div className="max-w-screen-2xl mx-auto">
              <div className="text-center">
                ...
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Button className="bg-gradient-to-r from-indigo-600 to-indigo-500 text-white px-8 py-4 text-lg rounded-lg hover:from-indigo-700 hover:to-indigo-600 transition-all shadow-md hover:shadow-lg" onClick={() => navigate("/users")}>
                    Manage Users Now
                  </Button>
                </motion.div>
              </div>
