...
import { Pencil, Trash, Search, Bell, Settings, X, ArrowLeft, LogOut, Send } from "lucide-react";
...
function GroupModal({ type, group, onClose, onSubmit }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        if (type === 'members' && group?.id) {
          const [currentMemberIds, usersData] = await Promise.all([
        ...
        } else if (type === 'edit' && group?.id) {
          const [groupDetails, groupEntitlements] = await Promise.all([
            getGroupDetails(group.id),
            getGroupEntitlements(group.id)
          ]);

          setGroupName(groupDetails.displayName || groupDetails.name);
          setEntitlements(groupEntitlements);
        } 
      } catch (err) {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex space-x-3">
              <Button variant="outline" className="flex-1 border-indigo-100 text-gray-700 hover:bg-indigo-50" onClick={onClose}>
                Cancel
              </Button>
              <Button variant="destructive" className="flex-1 bg-gradient-to-r from-red-600 to-red-500 text-white hover:from-red-700 hover:to-red-600 shadow-sm" onClick={handleSubmit}>
                Delete
              </Button>
            </div>
          </>
              ...
              <div className="max-h-96 overflow-y-auto mb-4 space-y-2">
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((user) => (
                    <motion.div 
                      key={user.id} 
                      className="flex items-center justify-between p-3 border border-indigo-50 rounded-lg hover:bg-indigo-50 transition-colors"
          ...
          <form onSubmit={handleSubmit}>
            <Input
              ...
              className="mb-4 border-indigo-100 focus:border-indigo-300 focus:ring-indigo-200"
            />
            <h3 className="text-lg font-medium text-gray-700 mb-2">Group Entitlements</h3>
            <div className="space-y-3 mb-6 border border-indigo-50 rounded-lg p-4 bg-indigo-50/30">
              <label className="flex items-center space-x-3 p-2 rounded-lg hover:bg-indigo-100/50 transition-colors">
                <Checkbox 
                  checked={entitlements.clusterCreation}
                  onCheckedChange={(checked) => setEntitlements({...entitlements, clusterCreation: checked})}
                  className="border-indigo-300 data-[state=checked]:bg-indigo-600"
                />
                <div>
                  <span className="text-gray-700 font-medium">Allow Unrestricted Cluster Creation</span>
                  <p className="text-xs text-gray-500">Allows users to create clusters without restrictions</p>
                </div>
              </label>
              <label className="flex items-center space-x-3 p-2 rounded-lg hover:bg-indigo-100/50 transition-colors">
                <Checkbox 
                  checked={entitlements.sqlAccess}
                  onCheckedChange={(checked) => setEntitlements({...entitlements, sqlAccess: checked})}
                  className="border-indigo-300 data-[state=checked]:bg-indigo-600"
                />
                <div>
                  <span className="text-gray-700 font-medium">Databricks SQL Access</span>
                  <p className="text-xs text-gray-500">Provides access to Databricks SQL workspace</p>
                </div>
              </label>
              <label className="flex items-center space-x-3 p-2 rounded-lg hover:bg-indigo-100/50 transition-colors">
                <Checkbox 
                  checked={entitlements.workspaceAccess}
                  onCheckedChange={(checked) => setEntitlements({...entitlements, workspaceAccess: checked})}
                  className="border-indigo-300 data-[state=checked]:bg-indigo-600"
                />
                <div>
                  <span className="text-gray-700 font-medium">Workspace Access</span>
                  <p className="text-xs text-gray-500">Grants access to the Databricks workspace</p>
                </div>
              </label>
            </div>
            <div className="flex space-x-3">
...
export default function UserGroupsPage() {
  ...
  const handleModalSubmit = async (data) => {
    ...
    if (!isOnline) {
      toast.error('Network error: You appear to be offline. Please check your connection.');
      return false;
    }
    ...
    try {
      ...
      if (modalType === 'members') {
        const currentMembers = await getGroupMembers(selectedGroup.id);
        const usersToAdd = data.members.filter(id => !currentMembers.includes(id));
        const usersToRemove = currentMembers.filter(id => !data.members.includes(id));
  
        await Promise.all([
          usersToAdd.length > 0 && addUsersToGroup(selectedGroup.id, usersToAdd),
          usersToRemove.length > 0 && removeUsersFromGroup(selectedGroup.id, usersToRemove)
        ]);
        ...
        const updates = {
          ...
          entitlements: {
            clusterCreation: data.entitlements?.clusterCreation || false,
            sqlAccess: data.entitlements?.sqlAccess || false,
            workspaceAccess: data.entitlements?.workspaceAccess || false
          }
        };
      ...
      console.error('Modal submit error:', {
        error: error.message,
        stack: error.stack,
        ...
        <div className="relative z-10">
          ...
          <div className="max-w-6xl mx-auto bg-white/90 backdrop-blur-sm p-6 rounded-xl border border-indigo-50 shadow-sm">
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-semibold text-gray-800 mb-2">Error Loading Groups</h3>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[33%] text-center text-lg font-semibold text-indigo-600">Group Name</TableHead>
                      <TableHead className="w-[33%] text-center text-lg font-semibold text-indigo-600">Actions</TableHead>
                      <TableHead className="w-[33%] text-center text-lg font-semibold text-indigo-600">Members</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredGroups.map((group) => (
                      ...
                      >
                        <TableCell className="font-medium w-[33%] text-center text-gray-800">
                          {group.name}
                        </TableCell>
            ...
            <Card className="p-6 bg-gradient-to-br from-indigo-50/50 to-indigo-100/20 border border-indigo-50">
              <CardContent className="text-center">
                ...
                <p className="text-gray-600 mt-2 mb-6">
                  Create and manage new user groups to streamline your user management process.
                </p>
                <motion.div
